...
function PaidClients() {
            const [paidClients, setpaidClients] = React.useState([]);

// const [fotoUrl, setFotoUrl] = React.useState('');
// const [openFoto, setOpenFoto] = React.useState(false);

React.useEffect(() => {
  ipcRenderer.send("send_paid_data");
  ipcRenderer.on("sending_paid_data", (event, data)=>{
    if(data){
     // const newClients = data.filter(client => client.valid === false)
   //  setNotPaidClients([...newClients])
   setpaidClients([...data])
     }
   })
}, [])
  ...
  return(
    ...
    <React.Fragment>
        ...
        <h1 style = {{color:'black', borderBottom:'2px solid gray'}}>Klientet Me Pagesë:  {paidClients.length}</h1>
